...
- **`INSTAGRAM_SESSION_ID`** (optional) – value of the `sessionid` cookie from a logged-in Instagram browser session. Required for private profiles and for `GET /instagram/user/stories` (stories are always login-gated); also helps avoid 429 rate-limit errors on public profiles. To obtain it: log into instagram.com in Chrome/Firefox → open DevTools → Application → Cookies → `instagram.com` → copy the value of `sessionid`. The `[api]` extra now includes `curl-cffi` which enables browser-level TLS impersonation (HTTP/2, correct cipher suites); re-run `pip install -e ".[api]"` to pick it up, as it is the primary defence against Instagram 429s on unauthenticated requests.
- **`TWITTER_AUTH_TOKEN`** (optional) – value of the `auth_token` cookie from a logged-in x.com session. Both `GET /twitter/tweet` and `GET /twitter/user/posts` work for public profiles without this. Injecting it may improve rate limits or unlock non-public content. To obtain it: log into x.com in Chrome/Firefox → DevTools → Application → Cookies → `x.com` → copy `auth_token`.
- **`TWITTER_CT0`** (optional, recommended alongside `TWITTER_AUTH_TOKEN`) – value of the `ct0` cookie from the same x.com session (CSRF token). Obtain it the same way as `TWITTER_AUTH_TOKEN` — same cookie jar, different key.
